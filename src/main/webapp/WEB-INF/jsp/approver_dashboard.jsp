...
<html>
    <head>
        ...
        <title>Fintrex - FMR - Approver</title>
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                ...
                                <input id="justification" type="text" name="justification" class="form-control" required autocomplete="off">
                            </div>
                        </div>
                    </div>
...
<!--                    
    ...
    <div class="comment-box">
        ...
        <p id="comment" class="comment-content">No Justification yet.</p>
    </div>
     <div class="comment-box">
        <label for="action">Action Taken:</label>
        <p id="action" class="comment-content">No action recorded.</p>
    </div>
    <div class="comment-box">
        <label for="mod_by_user">Action taken By:</label>
        <p id="mod_by_user" class="comment-content">Unknown</p>
    </div>
...
//                                    var justificationCell = newRow.insertCell(1);
                           ...
                           // Assuming modifiedOn is available in attachment object
   ...
   let attachments = data.videos; 
            ...
            if (attachments && attachments.length > 0) {
                attachments.forEach((attachment) => {
                    ...
                    // Set the text content of the cells with the fetched values
                    fileNameCell.textContent = attachment.name;
                    statusCell.textContent = attachment.status || 'N/A';
                    modifiedByCell.textContent = attachment.modby || 'N/A';
                    modifiedOnCell.textContent = attachment.mod_on || 'N/A';
                    dateCell.textContent = attachment.date || 'N/A';
                });
            } else {
                // No attachments found, you can handle it accordingly
                $('#tbladdAttu tbody').append('<tr><td colspan="5">No attachments available.</td></tr>');
            }
...
//                    +'<p>Modified On:'+(mod_on|| 'N/A')+'</p>'
 ...
 formData.justification = $('#justification').val();
