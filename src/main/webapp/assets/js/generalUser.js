...
var dtable = $('#viewAllComplaintasTableNewAdminDashboard').DataTable({
    ...
    "searchDelay": 350,
    "ajax": {
        "url": "table-data-allComplaintDataForGeneralUser",
...
//            $(action_td).append('<a href="javascript:void(0)" class="viewFullComplaintDetails"><i class="bi bi-eye" style="font-size:20px; color:blue"></i></a><a href="javascript:void(0)" class="reOpenComplaint" ><i class="bi bi-arrow-repeat" style="font-size:20px; margin-left:10px; color:orange"></i></a>');
            ...
            if (data['status'] === 'active' && data['status_repeat'] === 'Reopen') {
                ...
                $(row).find('td').eq(5).html('<label class="badge bg-primary" style="font-size:12px">Active</label>');
        ...
        body: JSON.stringify({filter: filter})
            ...
            .then(data => {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    var totalComplaintNewDashboardBranch = data[0].count_total_branch;
                    var progressComplaintNewDashboardBranch = data[0].count_progress_branch;

                    var rejectedComplaintNewDashboardBranch = data[0].count_rejected_branch;


                    var completeComplaintNewDashboardBranch = data[0].count_completed_branch;

                    var queueComplaintNewDashboardBranch = data[0].count_queue_branch;
                    var ownOfficerComplaintNewDashboardBranch = data[0].own_complaint;


                    document.getElementById('totalComplaintNewDashboard').textContent = totalComplaintNewDashboardBranch;
                    document.getElementById('completedComplaintNewDashboard').textContent = completeComplaintNewDashboardBranch;
                    document.getElementById('queueComplaintNewDashboard').textContent = queueComplaintNewDashboardBranch;
                    document.getElementById('progressComplaintNewDashboard').textContent = progressComplaintNewDashboardBranch;
                    document.getElementById('rejectedComplaintNewDashboard').textContent = rejectedComplaintNewDashboardBranch;
                    document.getElementById('ownComplaintNewDashboard').textContent = ownOfficerComplaintNewDashboardBranch;
